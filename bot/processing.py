...
def attack_multi_swap_exact_for(deadline, uniswap_data, gas_price):
	reserves, token0, pair_token0 = get_pair_info(uniswap_data)
	token0_in = False
	...
	if token0_in and token0 == WETH_ADDRESS:
		...
		if slippage > 0.05:
			...
			print("PROFIT:", profit)
			if profit > 0.01:
				multi_buy_tokens(attacker_amount_out, attacker_amount_in, [token0, token1], deadline, gas_price * 2, attacker_amount_in)
				multi_sell_tokens(attacker_amount_out, revenue, [token1, token0], deadline, gas_price, 0)
	elif not token0_in and token1 == WETH_ADDRESS:
...
def attack_multi_swap_for_exact(deadline, uniswap_data, gas_price):
	print(uniswap_data)
	reserves, token0, pair_token0 = get_pair_info(uniswap_data)
	token0_in = False
	if token0 == pair_token0:
		token0_in = True
		token1 = uniswap_data[1]["path"][1]
		token0_amount = uniswap_data[1]["amountInMax"]
		token1_amount = uniswap_data[1]["amountOut"]
	else:
		token0 = uniswap_data[1]["path"][1]
		token1 = uniswap_data[1]["path"][0]
		token0_amount = uniswap_data[1]["amountOut"]
		token1_amount = uniswap_data[1]["amountInMax"]
	token0_reserve = reserves[0]
	token1_reserve = reserves[1]
	if token0_in and token0 == WETH_ADDRESS:
		amount_in = calc_amount_in(token1_amount, token0_reserve, token1_reserve)
		slippage = 1 - amount_in / token0_amount
		print("Slippage:", slippage)
		if slippage > 0.05:
			attacker_amount_out = calc_attack_amount_out(token0_amount, token1_amount, token0_reserve, token1_reserve)
			attacker_amount_in = calc_amount_in(attacker_amount_out, token0_reserve, token1_reserve)
			print("Token0 reserve", token0_reserve)
			print("Token1 reserve", token1_reserve)
			print("Attacker amount IN:", attacker_amount_in)
			print("Attacker amount OUT:", attacker_amount_out)
			...
			if profit > 0.01:
				multi_buy_tokens(attacker_amount_out, attacker_amount_in, [token0, token1], deadline, gas_price * 2, attacker_amount_in)
				multi_sell_tokens(attacker_amount_out, revenue, [token1, token0], deadline, int(gas_price * 0.99), 0)
	elif not token0_in and token1 == WETH_ADDRESS:
		amount_in = calc_amount_in(token0_amount, token1_reserve, token0_reserve)
		slippage = 1 - amount_in / token1_amount
		print("Slippage:", slippage)
		if slippage > 0.05:
			attacker_amount_out = calc_attack_amount_out(token1_amount, token0_amount, token1_reserve, token0_reserve)
			attacker_amount_in = calc_amount_in(attacker_amount_out, token1_reserve, token0_reserve)
			print("Token0 reserve", token0_reserve)
			print("Token1 reserve", token1_reserve)
			print("Attacker amount IN:", attacker_amount_in)
			print("Attacker amount OUT:", attacker_amount_out)
			revenue = int(calc_revenue(attacker_amount_in, token1_amount, attacker_amount_out * 0.995, token0_amount, token1_reserve, token0_reserve) * 0.995)
			print("Revenue", revenue / 1e18)
			profit = (revenue - attacker_amount_in) / 1e18
			print("PROFIT:", profit)
			if profit > 0.01:
				multi_buy_tokens(int(attacker_amount_out * 0.995), attacker_amount_in, [token0, token1], deadline, gas_price * 2, attacker_amount_in)
				multi_sell_tokens(int(attacker_amount_out * 0.995), revenue, [token1, token0], deadline, int(gas_price * 0.99), 0)


def get_pair_info(uniswap_data):
	...
	return reserves, token0, pair_token0
...
def process_tx(tx):
	if tx["to"] == "0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45":
		if tx["input"][0:10] == MULTICALL_SELECTOR:
			...
			elif selector == SWAP_FOR_EXACT_SELECTOR:
				attack_multi_swap_for_exact(deadline, uniswap_data, gas_price)
